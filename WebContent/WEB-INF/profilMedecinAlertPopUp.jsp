<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
    
    <%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %>
    <%@ taglib uri="http://www.joda.org/joda/time/tags" prefix="joda" %>
<!DOCTYPE html >
<html>
<head>

<title>Profil Médecin</title>

<meta http-equiv="X-UA-Compatible" content="IE=edge" >
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
<meta name="description" content="Profil d'un médecin d' ENAD.">
<meta name="author" content="ENAD">

<link rel="shortcut icon" type="image/png" href="<c:url value="/inc/favicon.png"/>">
<!-- [if IE]><link rel="shortcut icon" type="image/x-icon" href="<c:url value="/inc/favicon.ico"/>"/><![endif]-->

<link href="<c:url value="/inc/bootstrap/css/bootstrap.css"/>" rel="stylesheet"/>

<link href="<c:url value="/inc/font-awesome/css/font-awesome.css"/>" rel="stylesheet"/>

<link href="<c:url value="/inc/style.css"/>" rel="stylesheet"/>

<!-- HTML5 Shim and Respond.js IE8 support of HTML5 elements and media queries -->
<!-- WARNING: Respond.js doesn't work if you view the page via file:// -->
<!-- [if lt IE 9]>
<script src="https://oss.maxcdn.com/libs/html5shiv/3.7.0/html5shiv.js"></script>
<script src="https://oss.maxcdn.com/libs/respond.js/1.3.0/respond.min.js"></script>
<! [endif]-->




</head>
<body>

<div class="container">


  <c:if test="${ ! empty actif  }">
  
       <c:if test="${  actif eq 'patient'  }">
       
       <c:import url="/inc/enadnotice.jsp"/>
              
              <div class="row">
      
            <div class="col-xs-12">
            
                    <div class="enad_ENAD">
                    
                    Profil du médecin 
                    
                    <c:if test="${ ! empty medecinprofil }">
${medecinprofil.prenoms_medecin }  ${medecinprofil.noms_medecin }


      
     <c:if test="${ ! empty medecinprofil.url_photo_medecin }">
     
     <br>
     
     <img class="" style="margin:auto;" width="200" height="200" id="img" alt="ENAD" src="http://${  sessionScope.adresseserveur }:8080/enad${  medecinprofil.url_photo_medecin }" >
                        
     
     
     </c:if>


</c:if>
                    
                    </div>
            </div>
      
      
      
      </div>
      
      
      <c:import url="/inc/presentationPatient.jsp"/>
      
      <br>
      
      <!-- MENU + CONTACTS + NOTIFS -->
      
      <div class="row">
      
           <!-- CONTACTS -->
           
           <c:import url="/inc/contactspat.jsp" />
           
           <!-- MENU -->
           
           <div class="col-xs-8">
           
                   <div class="menumed">
                   
                         <c:if test="${ ! empty liaisonmedpat }">
                         
                                <c:if test="${  liaisonmedpat eq 'oui' }">
                                
                                <c:if test="${ ! empty liaisonmedtraitantpat }">
                                
                                <c:if test="${  liaisonmedtraitantpat eq 'oui' }">
                                
                                     <div class="row relationavecmedecintraitantoui">
                                     
                                          Voici votre médecin traitant.
                                          
                                          <script type="text/javascript">
         
         var plusinfo = new SpeechSynthesisUtterance("Voici votre médecin traitant.");
         
         plusinfo.lang='fr-FR';
         
         window.speechSynthesis.speak(plusinfo);

         
         </script>
                                     
                                     
                                     </div>
                                
                                
                                
                                </c:if>
                                
                                <c:if test="${  liaisonmedtraitantpat eq 'non' }">
                                
                                   <div class="row relationavecmedecintraitantnon">
                                   
                                   <script type="text/javascript">
         
         var plusinf = new SpeechSynthesisUtterance("Vous êtes en relation avec ce médecin mais ce n'est pas votre médécin traitant.");
         
         plusinf.lang='fr-FR';
         
         window.speechSynthesis.speak(plusinf);

         
         </script>
                                     
                                          Vous êtes en relation avec ce médecin mais ce n'est pas votre médécin traitant.
                                     
                                     
                                     </div>
                                
                                
                                
                                <c:if test="${  !empty avoirmdt }">
                                
                                <c:if test="${  avoirmdt eq 'oui' }">
                                
                                </c:if>
                                
                                <c:if test="${  avoirmdt eq 'non' }">
                                
                                  <div class="row sansmedecintraitant">
                                  
                                       Vous n'avez pas de médecin traitant.
                                       
                                       <script type="text/javascript">
         
         var plusin = new SpeechSynthesisUtterance("Vous n'avez pas de médecin traitant.");
         
         plusin.lang='fr-FR';
         
         window.speechSynthesis.speak(plusin);

         
         </script>
                                  
                                  </div>
                                
                                
                                
                                                                                      <c:if test="${ ! empty demandeliaisonmedt }">
                                       
                                                       <c:if test="${  demandeliaisonmedt eq 'oui' }">
                                                       
                                                              <c:if test="${ ! empty demandeliaisonmedtsens }">
                                                              
                                                                       <c:if test="${  demandeliaisonmedtsens eq 'gauchedroite' }">
                                                                       
                                                                       
                                                                       
                                                                       <a class="btn btn-primary btn-lg btn-block "  href="<c:url value="/confirmation_medt_pat?ide_em=${medecinprofil.identifiant_medecin}&ide_dest=${sessionScope.sessionPatient.identifiant_patient }&page=${profil}"/>"><span class="glyphicon glyphicon-info-sign"></span> <br/>Je confirme la demande de mise en relation de ce médecin en tant que médecin traitant.</a>
                                                                       
                                                                       </c:if>
                                                                       
                                                                       <c:if test="${  demandeliaisonmedtsens eq 'droitegauche' }">
                                                                       
                                                                       <div class="row demandeattentevalidation">
                                                                       
                                                                       <script type="text/javascript">
         
         var plusi = new SpeechSynthesisUtterance("Votre demande de médecin traitant est en attente de validation par ce médecin.");
         
         plusi.lang='fr-FR';
         
         window.speechSynthesis.speak(plusi);

         
         </script>
                                                                       
                                                                       Votre demande de médecin traitant est en attente de validation par ce médecin.
                                                                       
                                                                       
                                                                       </div>
                                                                       
                                                                       
                                                                       </c:if>
                                                              
                                                              
                                                              
                                                              </c:if>
                                
                                
                                                       </c:if>
                                        
                                                      <c:if test="${  demandeliaisonmedt eq 'non' }">
                                                      
                                                      
                                                      
                                                      <a class="btn btn-primary btn-lg btn-block "  href="<c:url value="/etreenrelation_medt_pat?ide_em=${sessionScope.sessionPatient.identifiant_patient }&ide_dest=${medecinprofil.identifiant_medecin}&page=${profil}"/>"><span class="glyphicon glyphicon-info-sign"></span> <br/>Je souhaite que vous soyez mon médecin traitant.</a>
                                
                                
                                                      </c:if>
                                       
                                       
                                       </c:if>
      
                                
                                </c:if>
                                
                                
                                </c:if>
                                
                          
                                </c:if>
                                
                                </c:if>
                                
                                
                                
                                        
                                
                                
                                
                                </c:if>
                                
                                <c:if test="${  liaisonmedpat eq 'non' }">
                                
                                <div class="row relationavecmedecinnon">
                                
                                <script type="text/javascript">
         
         var plus = new SpeechSynthesisUtterance("Vous n'êtes pas en relation avec ce médecin");
         
         plus.lang='fr-FR';
         
         window.speechSynthesis.speak(plus);

         
         </script>
                                
                                   Vous n'êtes pas en relation avec ce médecin.
                                
                                
                                </div>
                                
                                
                                
                                
                                              <c:if test="${  ! empty demandeliaisonmedpat }">
                                              
                                              
                                                          <c:if test="${   demandeliaisonmedpat eq 'oui' }">
                                                          
                                                                 <c:if test="${  ! empty demandeliaisonmedpatsens }"> 
                                                                 
                                                                 
                                                                          <c:if test="${   demandeliaisonmedpatsens eq 'gauchedroite' }">
                                                                          
                                                                          
                                                                          
                                                                          <a class="btn btn-primary btn-lg btn-block "  href="<c:url value="/confirmation_med_pat?ide_em=${medecinprofil.identifiant_medecin}&ide_dest=${sessionScope.sessionPatient.identifiant_patient }&page=${profil}"/>"><span class="glyphicon glyphicon-info-sign"></span> <br/>Je confirme la demande de mise en relation de ce médecin.</a>
                                                                          
                                                                          </c:if>
                                                                          
                                                                          <c:if test="${   demandeliaisonmedpatsens eq 'droitegauche' }">
                                                                          
                                                                          <div class ="row demandeattentevalidation">
                                                                          
                                                                          <script type="text/javascript">
         
         var plu = new SpeechSynthesisUtterance("Votre demande est en attente de validation par ce médecin.");
         
         plu.lang='fr-FR';
         
         window.speechSynthesis.speak(plu);

         
         </script>
                                                                          
                                                                           Votre demande est en attente de validation par ce médecin.
                                                                          
                                                                          
                                                                          </div>
                                                                          
                                                                         
                                                                          
                                                                          </c:if>
                                                                 
                                                                 
                                                                         
                                                                 
                                                                 
                                                                 
                                                                 </c:if>
                                                                 
                                                                 
                                                          
                                                          </c:if>
                                                          
                                                          <c:if test="${   demandeliaisonmedpat eq 'non' }">
                                                          
                                                          
                                                          
                                                          <a class="btn btn-primary btn-lg btn-block "  href="<c:url value="/etreenrelation_med_pat?ide_em=${sessionScope.sessionPatient.identifiant_patient }&ide_dest=${medecinprofil.identifiant_medecin}&page=${profil}"/>"><span class="glyphicon glyphicon-info-sign"></span> <br/>Je souhaite être en relation avec ce médecin.</a>
                                                          
                                                          </c:if>
                                              
                                                      
                                              
                                              
                                              </c:if>
                                              
                                              <c:if test="${  !empty avoirmdt }">
                                
                                <c:if test="${  avoirmdt eq 'oui' }">
                                
                                
                                
                                </c:if>
                                
                                <c:if test="${  avoirmdt eq 'non' }">
                                
                                <div class="row sansmedecintraitant">
                                
                                <script type="text/javascript">
         
         var pl = new SpeechSynthesisUtterance("Vous n'avez pas de médecin traitant.");
         
         pl.lang='fr-FR';
         
         window.speechSynthesis.speak(pl);

         
         </script>
                                
                               
                                  
                                       Vous n'avez pas de médecin traitant.
                                  
                                  </div>
                                
                                                                                      <c:if test="${ ! empty demandeliaisonmedt }">
                                       
                                                       <c:if test="${  demandeliaisonmedt eq 'oui' }">
                                                       
                                                              <c:if test="${ ! empty demandeliaisonmedtsens }">
                                                              
                                                                       <c:if test="${  demandeliaisonmedtsens eq 'gauchedroite' }">
                                                                       
                                                                       
                                                                       
                                                                       <a class="btn btn-primary btn-lg btn-block "  href="<c:url value="/confirmation_medt_pat?ide_em=${medecinprofil.identifiant_medecin}&ide_dest=${sessionScope.sessionPatient.identifiant_patient }&page=${profil}"/>"><span class="glyphicon glyphicon-info-sign"></span> <br/>Je confirme la demande de mise en relation de ce médecin en tant que médecin traitant.</a>
                                                                       
                                                                       </c:if>
                                                                       
                                                                       <c:if test="${  demandeliaisonmedtsens eq 'droitegauche' }">
                                                                       
                                                                       <div class="row demandeattentevalidation">
                                                                       
                                                                       <script type="text/javascript">
         
         var p = new SpeechSynthesisUtterance("Votre demande de médecin traitant est en attente de validation par ce médecin.");
         
         p.lang='fr-FR';
         
         window.speechSynthesis.speak(p);

         
         </script>
                                                                       
                                                                       Votre demande de médecin traitant est en attente de validation par ce médecin.
                                                                       
                                                                       
                                                                       </div>
                                                                       
                                                                       </c:if>
                                                              
                                                              
                                                              
                                                              </c:if>
                                
                                
                                                       </c:if>
                                        
                                                      <c:if test="${  demandeliaisonmedt eq 'non' }">
                                                      
                                                   
                                                      
                                                      <a class="btn btn-primary btn-lg btn-block "  href="<c:url value="/etreenrelation_medt_pat?ide_em=${sessionScope.sessionPatient.identifiant_patient }&ide_dest=${medecinprofil.identifiant_medecin}&page=${profil}"/>"><span class="glyphicon glyphicon-info-sign"></span> <br/>Je souhaite que vous soyez mon médecin traitant.</a>
                                
                                
                                                      </c:if>
                                       
                                       
                                       </c:if>
      
                                
                                </c:if>
                                
                                
                                </c:if>
                                
                                
                                
                                       
                                        
                                
                                
                                </c:if>
                         
                         
                         </c:if>
                   
                   
                   </div>
           
           
           </div>
           
           <!-- NOTIFS  -->

			<c:import url="/inc/notifspat.jsp" />
           
      
      
      </div>
      
      <br /> <br /> <br /> <br /> <br /> <br /> <br /> <br /> <br />
              
              

<c:import url="/inc/footer.jsp"/>
                                    
                                    
                               
       
       
       
       </c:if>
       
       <c:if test="${  actif eq 'medecin'  }">
       
       
              <c:if test="${ ! empty action  }">
            
                  
                   <c:if test="${  action eq 'consulteretmodifier'  }">
                   
                           <c:import url="/inc/enadnotice.jsp"/>
              
              <div class="row">
      
            <div class="col-xs-12">
            
            <div id="white-background-form" style="display:none;
    width:100%;
    height:100%;
    position:fixed;
    top:0px;
    left:0px;
    background-color:#fefefe;
    opacity:0.7;
    z-index:9999;">
            </div>
            
                   <div id="dlgbox_nomsmed-form" style="display:none;
  
   position:absolute;
    width:600px;
    z-index:999999;
    border-radius:10px;
    
    
    right:30%;
    
    top:20%;
    
    left:30%;
    
    border: 10px #0099cc outset;">
    
                                  <div id="dlg-header_nomsmed-form" style="background-color:#0099cc;
  color:white;
  font-size:20px;
  padding:10px;
  margin: 10px 10px 0px 10px;
  "> Je modifie mon nom ou mes noms.
  
  <button class="close"  id="fermerhautnomsmed"><span  class="glyphicon glyphicon-remove-circle"></span></button>
  
                                  
                                  </div>
                                  
                                  <div id="dlg-body_nomsmed-form" style="background-color:white;
  
  color:black;
  
  font-size:14px;
  
  padding:10px;
  
  margin:0px 10px 0px 10px;">
  
         <div class=" well">
                                 
                                         <div class="row">
                                         
                                               <div class="col-xs-12">
                                               
                                                    <div class="enad_ins_med">
                                                    
                                                         <form method="post" class="col-xs-12 "
						action="<c:url value="/modification_noms_Medecin"/>">
									                                         
									                                              <fieldset>
									                                                
									                                                <br />
									                                                
									                                                    <div class="form-group">
									                                                    
									                                                          <p class="titres_ins_medecin">Nom(s) du médecin</p>

								<label for="nomsMedecin"></label> <input
									class="form-control input-lg" type="text" id="nomsMedecin"
									name="nomsMedecin"
									
									value="<c:out value="${medecin.noms_medecin }"/>" required />


								


									                                                          
									                                                    
									                                                    
									                                                    </div>
									                                                    
									                                                    <br>
									                                              
									                                              <div class="form-group">
									                      
									                         <button class="btn btn-primary btn-lg btn-block " type="reset">
									<span class="glyphicon glyphicon-erase"></span>
									<h4>J'efface  la donnée du formulaire.</h4>
								</button>
								
								<br>
								
                                                               
                                                               <button class="btn btn-success btn-lg btn-block "  type="submit">
									<span class="glyphicon glyphicon-ok-sign"></span>
									<h4>Je modifie mon nom.</h4>
								</button>
								
								
								
								             
									     </div>
									     
									     
                                       
									                                              
									                                              </fieldset>
									                                              
									                                              
									                                         
									                                         
									                                         
									                                         </form>
                                                    
                                                    </div>
                                               
                                               </div>
                                               
                                         </div>
                                         
          </div>                                     
                                  
                                  </div>
                                  
                                  <div id="dlg-footer_nomsmed-form" style="background-color:#f2f2f2;
  
  text-align:right;
  padding:10px;
  margin:0px 10px 10px 10px;">
  
                                  <br>
								
								<button class="btn btn-info btn-lg btn-block " id="fermerdlgboxnomsmed" >
									<span class="glyphicon glyphicon-remove-circle"></span>
									<h4>Fermer</h4>
								</button>
                                  
                                  </div>
                   
                   </div >
                   
                   <div id="dlgbox_nomsmed-form-succes" style="display:none;
  
   position:fixed;
    width:600px;
    z-index:999999;
    border-radius:10px;
    
    
    right:30%;
    
    top:20%;
    
    left:30%;
    
    border: 10px #2ecc71 outset;">
            
                 <div id="dlg-header_nomsmed-form-succes" style="background-color:#2ecc71;
  color:white;
  font-size:20px;
  padding:10px;
  margin: 10px 10px 0px 10px;">Succès de la modification de vos noms / votre nom</div>
                 
                 
                 
                 <div id="dlg-body_nomsmed-form-succes" style="background-color:white;
  
  color:black;
  
  font-size:14px;
  
  padding:10px;
  
  margin:0px 10px 0px 10px;">
  
                              <div style="background-color:#2ecc71; text-align:center; border-radius:10px;">
                              
                                    <br />
                                    
                                    <strong style=" font-size: 2em; color:white; ">  <i class="fa fa-check" aria-hidden="true"></i></strong> <br />
					<br />
                              
                              
                              </div>
                 
                 </div>
                 
                 <div id="dlg-footer_nomsmed-form-succes" style="background-color:#f2f2f2;
  
  text-align:right;
  padding:10px;
  margin:0px 10px 10px 10px;">
  
                           <button id="fermer_noms_succes" style="color: white;
  background-color: #2ecc71;
  border-color: #ecf0f1;" class="btn ">Fermer</button>
             
                 
                 </div>
            
            
            
            
            </div>
            
            <c:if test="${! empty  succes_noms }">
            
            <script src="<c:url value="/inc/jquery-1.12.4.min.js"/>"></script>
            
            
            <script type="text/javascript">
            
            	
				
				   document.getElementById("white-background-form").style.display ="block";
				   
				   document.getElementById("dlgbox_nomsmed-form-succes").style.display ="block";
				   
				   $('#fermer_noms_succes').click(function(){
						
					   document.getElementById("white-background-form").style.display ="none";
					   
					   document.getElementById("dlgbox_nomsmed-form-succes").style.display ="none";
					   
					   
				   });
				   
				   


				   var noms_succes = new SpeechSynthesisUtterance("Succès de la modification de la rubrique noms");

				   noms_succes.lang="fr-FR";
				   window.speechSynthesis.speak(noms_succes);
				   
				   


				   
            
            </script>
            
            
            </c:if>
                   
                   <div id="dlgbox_prenomsmed-form" style="display:none;
  
   position:absolute;
    width:600px;
    z-index:999999;
    border-radius:10px;
    
    
    right:30%;
    
    top:20%;
    
    left:30%;
    
    border: 10px #0099cc outset;">
                   
                          <div id="dlg-header_prenomsmed-form" style="background-color:#0099cc;
  color:white;
  font-size:20px;
  padding:10px;
  margin: 10px 10px 0px 10px;
  ">Je modifie mon prénom ou mes prénoms.
  
  <button class="close"  id="fermerhautprenomsmed"><span  class="glyphicon glyphicon-remove-circle"></span></button>
  
                          
                          </div>
                                  
                                  <div id="dlg-body_prenomsmed-form" style="background-color:white;
  
  color:black;
  
  font-size:14px;
  
  padding:10px;
  
  margin:0px 10px 0px 10px;">
  
              <div class=" well">
                                 
                                         <div class="row">
                                         
                                               <div class="col-xs-12">
                                               
                                                    <div class="enad_ins_med">
                                                    
                                                    <form method="post" class="col-xs-12 "
						action="<c:url value="/modification_prenoms_Medecin"/>">
									                                         
									                                              <fieldset>
									                                                
									                                                <br />
									                                                
									                                                    <div class="form-group">
									                                                    
									                                                          <p class="titres_ins_medecin">Prénom(s) du médecin</p>

								<label for="prenomsMedecin"></label> <input
									class="form-control input-lg" type="text" id="prenomsMedecin"
									name="prenomsMedecin"
									
									value="<c:out value="${medecin.prenoms_medecin }"/>"  />


								


									                                                          
									                                                    
									                                                    
									                                                    </div>
									                                                    
									                                                    <br>
									                                              
									                                              <div class="form-group">
									                      
									                         <button class="btn btn-primary btn-lg btn-block " type="reset">
									<span class="glyphicon glyphicon-erase"></span>
									<h4>J'efface  la donnée du formulaire.</h4>
								</button>
								
								<br>
								
                                                               
                                                               <button class="btn btn-success btn-lg btn-block "  type="submit">
									<span class="glyphicon glyphicon-ok-sign"></span>
									<h4>Je modifie mon prénom.</h4>
								</button>
								
								
								              
									                   
									     </div>
									     
									     
                                       
									                                              
									                                              </fieldset>
									                                              
									                                              
									                                         
									                                         
									                                         
									                                         </form>
                                                    
                                                    </div>
                                  
                                              </div>
                                              
                                          </div>
                                          
              </div>                                
                                  </div>
                                  
                                  <div id="dlg-footer_prenomsmed-form" style="background-color:#f2f2f2;
  
  text-align:right;
  padding:10px;
  margin:0px 10px 10px 10px;">
  
                               <br>
								
								<button class="btn btn-info btn-lg btn-block " id="fermerdlgboxprenomsmed" >
									<span class="glyphicon glyphicon-remove-circle"></span>
									<h4>Fermer</h4>
								</button>
                                  
                                  </div>
                   
                   </div>
                   
                   <div id="dlgbox_prenomsmed-form-succes" style="display:none;
  
   position:fixed;
    width:600px;
    z-index:999999;
    border-radius:10px;
    
    
    right:30%;
    
    top:20%;
    
    left:30%;
    
    border: 10px #2ecc71 outset;">
            
                 <div id="dlg-header_prenomsmed-form-succes" style="background-color:#2ecc71;
  color:white;
  font-size:20px;
  padding:10px;
  margin: 10px 10px 0px 10px;">Succès de la modification de vos prénoms / votre prénom</div>
                 
                 
                 
                 <div id="dlg-body_prenomsmed-form-succes" style="background-color:white;
  
  color:black;
  
  font-size:14px;
  
  padding:10px;
  
  margin:0px 10px 0px 10px;">
  
                              <div style="background-color:#2ecc71; text-align:center; border-radius:10px;">
                              
                                    <br />
                                    
                                    <strong style=" font-size: 2em; color:white; ">  <i class="fa fa-check" aria-hidden="true"></i></strong> <br />
					<br />
                              
                              
                              </div>
                 
                 </div>
                 
                 <div id="dlg-footer_prenomsmed-form-succes" style="background-color:#f2f2f2;
  
  text-align:right;
  padding:10px;
  margin:0px 10px 10px 10px;">
  
                           <button id="fermer_prenoms_succes" style="color: white;
  background-color: #2ecc71;
  border-color: #ecf0f1;" class="btn ">Fermer</button>
             
                 
                 </div>
            
            
            
            
            </div>
            
            <c:if test="${! empty  succes_prenoms }">
            
            <script src="<c:url value="/inc/jquery-1.12.4.min.js"/>"></script>
            
            
            <script type="text/javascript">
            
            	
				
				   document.getElementById("white-background-form").style.display ="block";
				   
				   document.getElementById("dlgbox_prenomsmed-form-succes").style.display ="block";
				   
				   $('#fermer_prenoms_succes').click(function(){
						
					   document.getElementById("white-background-form").style.display ="none";
					   
					   document.getElementById("dlgbox_prenomsmed-form-succes").style.display ="none";
					   
					   
				   });
				   
				   var prenoms_succes = new SpeechSynthesisUtterance("Succès de la modification de la rubrique prénoms");

				   prenoms_succes.lang="fr-FR";
				   window.speechSynthesis.speak(prenoms_succes);
				    
            
            </script>
            
            
            </c:if>
                   
                   <div id="dlgbox_identifiantmed-form"  style="display:none;
  
   position:absolute;
    width:600px;
    z-index:999999;
    border-radius:10px;
    
    
    right:30%;
    
    top:20%;
    
    left:30%;
    
    border: 10px #0099cc outset;">
                   
                      <div id="dlg-header_identifiantmed-form" style="background-color:#0099cc;
  color:white;
  font-size:20px;
  padding:10px;
  margin: 10px 10px 0px 10px;
  "  > Je modifie mon identifiant.
  
  <button class="close"  id="fermerhautidentifiantmed"><span  class="glyphicon glyphicon-remove-circle"></span></button>
  
                      
                      </div>
                                  
                                  <div id="dlg-body_identifiantmed-form" style="background-color:white;
  
  color:black;
  
  font-size:14px;
  
  padding:10px;
  
  margin:0px 10px 0px 10px;">
  
                  <div class=" well">
                                 
                                         <div class="row">
                                         
                                               <div class="col-xs-12">
                                               
                                                    <div class="enad_ins_med">
                                                    
                                                         <form method="post" class="col-xs-12 " action="<c:url value="/modification_identifiant_Medecin"/>">
									                                            
									                                            
									                                                  <fieldset>
									                                                  
									                                                       <br>
									                                                       
									                                                       <div class="form-group">
									                                                       
									                                                             <p class="titres_ins_medecin">Identifiant du médecin</p>
									                                                             
									                                                             <label for="identifiantMedecin"></label> 
									                                                             
									                                                             <input
									                                                                class="form-control input-lg" type="text"
									                                                                id="identifiantMedecin" name="identifiantMedecin"
									                                                                
									                                                                value="<c:out value="${medecin.identifiant_medecin }"/>"
									                                                                required />
									                                                                
									                                                                
									                                                                
									                                                                
									                                                             
									                                                       
									                                                       
									                                                       </div>
									                                                       
									                                                       <br>
									                                                       
									                                                         <div class="form-group">

								<p class="titres_ins_medecin">Confirmation de l'identifiant
									du médecin</p>

								<label for="confirmeidentifiantMedecin"></label> <input
									class="form-control input-lg" type="text"
									id="confirmeidentifiantMedecin"
									name="confirmeidentifiantMedecin"
									
									value="<c:out value=""/>" required />

								




							</div>
							
							<br>
							
							<div class="form-group">
									                      
									                         <button class="btn btn-primary btn-lg btn-block " type="reset">
									<span class="glyphicon glyphicon-erase"></span>
									<h4>J'efface  les données du formulaire.</h4>
								</button>
								
								<br>
								
                                                               
                                                               <button class="btn btn-success btn-lg btn-block "  type="submit">
									<span class="glyphicon glyphicon-ok-sign"></span>
									<h4>Je modifie mon identifiant.</h4>
								</button>
								                 
									     </div>
									     
									                                                         
									                                                  
									                                                  
									                                                  </fieldset>
									                                            
									                                            
									                                            </form>
                                                    
                                                    </div>
                                                    
                                                 </div>
                                                 
                                            </div>
                                            
                    </div>                                
                                  
                                  </div>
                                  
                                  <div id="dlg-footer_identifiantmed-form" style="background-color:#f2f2f2;
  
  text-align:right;
  padding:10px;
  margin:0px 10px 10px 10px;">
  
                       <br>
								
								<button class="btn btn-info btn-lg btn-block " id="fermerdlgboxidentifiantmed" >
									<span class="glyphicon glyphicon-remove-circle"></span>
									<h4>Fermer</h4>
								</button>
                                  
                                  </div>
                   
                   </div>
                   
                   <div id="dlgbox_identifiantmed-form-succes" style="display:none;
  
   position:fixed;
    width:600px;
    z-index:999999;
    border-radius:10px;
    
    
    right:30%;
    
    top:20%;
    
    left:30%;
    
    border: 10px #2ecc71 outset;">
            
                 <div id="dlg-header_identifiantmed-form-succes" style="background-color:#2ecc71;
  color:white;
  font-size:20px;
  padding:10px;
  margin: 10px 10px 0px 10px;">Succès de la modification de l'identifiant</div>
                 
                 
                 
                 <div id="dlg-body_identifiantmed-form-succes" style="background-color:white;
  
  color:black;
  
  font-size:14px;
  
  padding:10px;
  
  margin:0px 10px 0px 10px;">
  
                              <div style="background-color:#2ecc71; text-align:center; border-radius:10px;">
                              
                                    <br />
                                    
                                    <strong style=" font-size: 2em; color:white; ">  <i class="fa fa-check" aria-hidden="true"></i></strong> <br />
					<br />
                              
                              
                              </div>
                 
                 </div>
                 
                 <div id="dlg-footer_identifiantmed-form-succes" style="background-color:#f2f2f2;
  
  text-align:right;
  padding:10px;
  margin:0px 10px 10px 10px;">
  
                           <button id="fermer_identifiant_succes" style="color: white;
  background-color: #2ecc71;
  border-color: #ecf0f1;" class="btn ">Fermer</button>
             
                 
                 </div>
            
            
            
            
            </div>
            
            <c:if test="${! empty  succes_identifiant }">
            
            <script src="<c:url value="/inc/jquery-1.12.4.min.js"/>"></script>
            
            
            <script type="text/javascript">
            
            	
				
				   document.getElementById("white-background-form").style.display ="block";
				   
				   document.getElementById("dlgbox_identifiantmed-form-succes").style.display ="block";
				   
				   $('#fermer_identifiant_succes').click(function(){
						
					   document.getElementById("white-background-form").style.display ="none";
					   
					   document.getElementById("dlgbox_identifiantmed-form-succes").style.display ="none";
					   
					   
				   });
				   
				   var identifiant_succes = new SpeechSynthesisUtterance("Succès de la modification de l'identifiant");

				   identifiant_succes.lang="fr-FR";
				   window.speechSynthesis.speak(identifiant_succes);
				    
            
            </script>
            
            
            </c:if>
            
            <div id="dlgbox_identifiantmed-form-echec-espace" style="display:none;
  
   position:fixed;
    width:600px;
    z-index:999999;
    border-radius:10px;
    
    
    right:30%;
    
    top:20%;
    
    left:30%;
    
    border: 10px #E74C3C outset;">
            
                 <div id="dlg-header_identifiantmed-form-echec-espace" style="background-color:#E74C3C;
  color:white;
  font-size:20px;
  padding:10px;
  margin: 10px 10px 0px 10px;">Échec de la modification de l'identifiant</div>
                 
                 <div id="dlg-body_identifiantmed-form-echec-espace" style="background-color:white;
  
  color:white;
  
  font-size: 1.5em;
  
  
  
  padding:10px;
  
  margin:0px 10px 0px 10px;">
  
  
        <div style="background-color:#E74C3C; text-align:center; border-radius:10px;">
        <br>
        
        Cause : Espace présent dans l'identifiant
                              
                                    <br />
                                    
                                    <strong style=" font-size: 2em; color:white; ">  <i class="fa fa-times" aria-hidden="true"></i></strong> <br />
					<br />
                              
                              
                              </div>
  
  
  </div>
                 
                 <div id="dlg-footer_identifiantmed-form-echec-espace" style="background-color:#f2f2f2;
  
  text-align:right;
  padding:10px;
  margin:0px 10px 10px 10px;">
  
              <button id="fermer_identifiant_echec-espace" style="color: white;
  background-color: #E74C3C;
  border-color: #E74C3C;" class="btn ">Fermer</button> 
  
  </div>
            
            
            
            
            </div>
            
             <c:if test="${! empty  espace }">
             
             <script src="<c:url value="/inc/jquery-1.12.4.min.js"/>"></script>
            
            <script type="text/javascript">
            
            
            
            	
				
				   document.getElementById("white-background-form").style.display ="block";
				   
				   document.getElementById("dlgbox_identifiantmed-form-echec-espace").style.display ="block";
				   
				   $('#fermer_identifiant_echec-espace').click(function(){
						
					   document.getElementById("white-background-form").style.display ="none";
					   
					   document.getElementById("dlgbox_identifiantmed-form-echec-espace").style.display ="none";
					   
					   
				   });
				   
				   var identifiant_echec_espace_un = new SpeechSynthesisUtterance("Échec de la modification de l'identifiant");

				   
				   
				   var identifiant_echec_espace_deux = new SpeechSynthesisUtterance("Cause : Espace présent dans l'identifiant");

				   identifiant_echec_espace_deux.lang="fr-FR";
				   
				   identifiant_echec_espace_un.lang="fr-FR";
				   
				   
				   window.speechSynthesis.speak(identifiant_echec_espace_un);
				   
				   window.speechSynthesis.speak(identifiant_echec_espace_deux);
				    
            
            </script>
            
            
            </c:if>
            
            <div id="dlgbox_identifiantmed-form-echec-difference" style="display:none;
  
   position:fixed;
    width:600px;
    z-index:999999;
    border-radius:10px;
    
    
    right:30%;
    
    top:20%;
    
    left:30%;
    
    border: 10px #E74C3C outset;">
    
    <div id="dlg-header_identifiantmed-form-echec-difference" style="background-color:#E74C3C;
  color:white;
  font-size:20px;
  padding:10px;
  margin: 10px 10px 0px 10px;">Échec de la modification de l'identifiant</div>
                 
                 <div id="dlg-body_identifiantmed-form-echec-difference" style="background-color:white;
  
  color:white;
  
  font-size: 1.5em;
  
  
  
  padding:10px;
  
  margin:0px 10px 0px 10px;">
  
  
        <div style="background-color:#E74C3C; text-align:center; border-radius:10px;">
        <br>
        
        Cause : Les identifiants saisis sont différents.
                              
                                    <br />
                                    
                                    <strong style=" font-size: 2em; color:white; ">  <i class="fa fa-times" aria-hidden="true"></i></strong> <br />
					<br />
                              
                              
                              </div>
  
  
  </div>
                 
                 <div id="dlg-footer_identifiantmed-form-echec-difference" style="background-color:#f2f2f2;
  
  text-align:right;
  padding:10px;
  margin:0px 10px 10px 10px;">
  
              <button id="fermer_identifiant_echec-difference" style="color: white;
  background-color: #E74C3C;
  border-color: #E74C3C;" class="btn ">Fermer</button> 
  
  </div>
            
            
            
            
            
            
            
            
            </div>
            
            <c:if test="${! empty  difference }">
            
            <script src="<c:url value="/inc/jquery-1.12.4.min.js"/>"></script>
            
            <script type="text/javascript">
            
            	
				
				   document.getElementById("white-background-form").style.display ="block";
				   
				   document.getElementById("dlgbox_identifiantmed-form-echec-difference").style.display ="block";
				   
				   $('#fermer_identifiant_echec-difference').click(function(){
						
					   document.getElementById("white-background-form").style.display ="none";
					   
					   document.getElementById("dlgbox_identifiantmed-form-echec-difference").style.display ="none";
					   
					   
				   });
				   
                    var identifiant_echec_difference_un = new SpeechSynthesisUtterance("Échec de la modification de l'identifiant");

				   
				   var identifiant_echec_difference_deux = new SpeechSynthesisUtterance("Cause : Les identifiants saisis sont différents.");

				   identifiant_echec_difference_deux.lang="fr-FR";
				   
				   identifiant_echec_difference_un.lang="fr-FR";
				   
				   
				   window.speechSynthesis.speak(identifiant_echec_difference_un);
				   
				   window.speechSynthesis.speak(identifiant_echec_difference_deux);
				    
            
            </script>
            
            
            </c:if>
            
            <div id="dlgbox_identifiantmed-form-echec-quatre" style="display:none;
  
   position:fixed;
    width:600px;
    z-index:999999;
    border-radius:10px;
    
    
    right:30%;
    
    top:20%;
    
    left:30%;
    
    border: 10px #E74C3C outset;">
    
    <div id="dlg-header_identifiantmed-form-echec-quatre" style="background-color:#E74C3C;
  color:white;
  font-size:20px;
  padding:10px;
  margin: 10px 10px 0px 10px;">Échec de la modification de l'identifiant</div>
                 
                 <div id="dlg-body_identifiantmed-form-echec-quatre" style="background-color:white;
  
  color:white;
  
  font-size: 1.5em;
  
  
  
  padding:10px;
  
  margin:0px 10px 0px 10px;">
  
  
        <div style="background-color:#E74C3C; text-align:center; border-radius:10px;">
        <br>
        
        Cause : Les identifiants contiennent moins de 4 caractères.
                              
                                    <br />
                                    
                                    <strong style=" font-size: 2em; color:white; ">  <i class="fa fa-times" aria-hidden="true"></i></strong> <br />
					<br />
                              
                              
                              </div>
  
  
  </div>
                 
                 <div id="dlg-footer_identifiantmed-form-echec-quatre" style="background-color:#f2f2f2;
  
  text-align:right;
  padding:10px;
  margin:0px 10px 10px 10px;">
  
              <button id="fermer_identifiant_echec-quatre" style="color: white;
  background-color: #E74C3C;
  border-color: #E74C3C;" class="btn ">Fermer</button> 
  
  </div>
            
            
            
            
            
            
            
            </div>
            
            <c:if test="${! empty  quatre }">
            
            <script src="<c:url value="/inc/jquery-1.12.4.min.js"/>"></script>
            
            <script type="text/javascript">
            
            	
				
				   document.getElementById("white-background-form").style.display ="block";
				   
				   document.getElementById("dlgbox_identifiantmed-form-echec-quatre").style.display ="block";
				   
				   $('#fermer_identifiant_echec-quatre').click(function(){
						
					   document.getElementById("white-background-form").style.display ="none";
					   
					   document.getElementById("dlgbox_identifiantmed-form-echec-quatre").style.display ="none";
					   
					   
				   });
				   
                  var identifiant_echec_quatre_un = new SpeechSynthesisUtterance("Échec de la modification de l'identifiant");

				   var identifiant_echec_quatre_deux = new SpeechSynthesisUtterance("Cause : Les identifiants contiennent moins de 4 caractères.");

				   identifiant_echec_quatre_deux.lang="fr-FR";
				   
				   identifiant_echec_quatre_un.lang="fr-FR";
				   
				   
				   window.speechSynthesis.speak(identifiant_echec_quatre_un);
				   
				   window.speechSynthesis.speak(identifiant_echec_quatre_deux);
				    
				    
            
            </script>
            
            
            </c:if>
            
            <div id="dlgbox_identifiantmed-form-echec-unique" style="display:none;
  
   position:fixed;
    width:600px;
    z-index:999999;
    border-radius:10px;
    
    
    right:30%;
    
    top:20%;
    
    left:30%;
    
    border: 10px #E74C3C outset;">
    
    <div id="dlg-header_identifiantmed-form-echec-unique" style="background-color:#E74C3C;
  color:white;
  font-size:20px;
  padding:10px;
  margin: 10px 10px 0px 10px;">Échec de la modification de l'identifiant</div>
                 
                 <div id="dlg-body_identifiantmed-form-echec-quatre" style="background-color:white;
  
  color:white;
  
  font-size: 1.5em;
  
  
  
  padding:10px;
  
  margin:0px 10px 0px 10px;">
  
  
        <div style="background-color:#E74C3C; text-align:center; border-radius:10px;">
        <br>
        
        Cause : L'identifiant est déjà utilisé.
                              
                                    <br />
                                    
                                    <strong style=" font-size: 2em; color:white; ">  <i class="fa fa-times" aria-hidden="true"></i></strong> <br />
					<br />
                              
                              
                              </div>
  
  
  </div>
                 
                 <div id="dlg-footer_identifiantmed-form-echec-unique" style="background-color:#f2f2f2;
  
  text-align:right;
  padding:10px;
  margin:0px 10px 10px 10px;">
  
              <button id="fermer_identifiant_echec-unique" style="color: white;
  background-color: #E74C3C;
  border-color: #E74C3C;" class="btn ">Fermer</button> 
  
  </div>
            
            
            
            
            
            
            
            
            
            </div>
            
            <c:if test="${! empty  unique }">
            
            <script src="<c:url value="/inc/jquery-1.12.4.min.js"/>"></script>
            
            <script type="text/javascript">
            
            	
				
				   document.getElementById("white-background-form").style.display ="block";
				   
				   document.getElementById("dlgbox_identifiantmed-form-echec-unique").style.display ="block";
				   
				   $('#fermer_identifiant_echec-unique').click(function(){
						
					   document.getElementById("white-background-form").style.display ="none";
					   
					   document.getElementById("dlgbox_identifiantmed-form-echec-unique").style.display ="none";
					   
					   
				   });
				   
				   var identifiant_echec_unique_un = new SpeechSynthesisUtterance("Échec de la modification de l'identifiant");

				   var identifiant_echec_unique_deux = new SpeechSynthesisUtterance("Cause : L'identifiant est déjà utilisé.");

				   identifiant_echec_unique_deux.lang="fr-FR";
				   
				   identifiant_echec_unique_un.lang="fr-FR";
				   
				   
				   window.speechSynthesis.speak(identifiant_echec_unique_un);
				   
				   window.speechSynthesis.speak(identifiant_echec_unique_deux);
				    
				    
            
            </script>
            
            
            </c:if>
                   
                   <div id="dlgbox_mdpmed-form" style="display:none;
  
   position:absolute;
    width:600px;
    z-index:999999;
    border-radius:10px;
    
    
    right:30%;
    
    top:20%;
    
    left:30%;
    
    border: 10px #0099cc outset;">
                   
                        <div id="dlg-header_mdpmed-form" style="background-color:#0099cc;
  color:white;
  font-size:20px;
  padding:10px;
  margin: 10px 10px 0px 10px;
  "> Je modifie mon mot de passe.
  
  <button class="close"  id="fermerhautmdpmed"><span  class="glyphicon glyphicon-remove-circle"></span></button>
  
                        
                        </div>
                                  
                                  <div id="dlg-body_mdpmed-form">
                                  
                                         <div class=" well">
                                 
                                         <div class="row">
                                         
                                               <div class="col-xs-12">
                                               
                                                    <div class="enad_ins_med">
                                                    
                                                         <form method="post" class="col-xs-12 " action="<c:url value="/modification_mdp_Medecin"/>">
									                                             
									                                             
									                                                 <fieldset>
									                                                 
									                                                    <br>
									                                                    
									                                                    
							<div class="form-group">

								<p class="titres_ins_medecin">Mot de passe du médecin</p>

								<label for="mdpMedecin"></label> <input
									class="form-control input-lg" type="password" id="mdpMedecin"
									name="mdpMedecin"
									
									value="" required />


								


							</div>

							<br />

							<div class="form-group">

								<p class="titres_ins_medecin">Confirmation du mot de passe
									du médecin</p>

								<label for="confirmemdpMedecin"></label> <input
									class="form-control input-lg" type="password"
									id="confirmemdpMedecin" name="confirmemdpMedecin"
									
									value="" required />

								



							</div>
							
								
							<br>
							
							<div class="form-group">
									                      
									                         <button class="btn btn-primary btn-lg btn-block " type="reset">
									<span class="glyphicon glyphicon-erase"></span>
									<h4>J'efface  les données du formulaire.</h4>
								</button>
								
								<br>
								
                                                               
                                                               <button class="btn btn-success btn-lg btn-block "  type="submit">
									<span class="glyphicon glyphicon-ok-sign"></span>
									<h4>Je modifie mon mot de passe.</h4>
								</button>
								
								
								               
									     </div>
									
									                                                    
									                                                 
									                                                 
									                                                 
									                                                 
									                                                 </fieldset>
									                                             
									                                             
									                                             
									                                             </form>
                                                    
                                                    </div>
                                                    
                                               </div>
                                               
                                             </div>
                                             
                                          </div>          
                                  
                                  </div>
                                  
                                  <div id="dlg-footer_mdpmed-form" style="background-color:#f2f2f2;
  
  text-align:right;
  padding:10px;
  margin:0px 10px 10px 10px;">
  
          <br>
								
								<button class="btn btn-info btn-lg btn-block " id="fermerdlgboxmdpmed" >
									<span class="glyphicon glyphicon-remove-circle"></span>
									<h4>Fermer</h4>
								</button>
                                  
                                  </div>
                   
                   
                   </div>
                   
                   <div id="dlgbox_mdpmed-form-succes" style="display:none;
  
   position:fixed;
    width:600px;
    z-index:999999;
    border-radius:10px;
    
    
    right:30%;
    
    top:20%;
    
    left:30%;
    
    border: 10px #2ecc71 outset;">
            
                 <div id="dlg-header_mdpmed-form-succes" style="background-color:#2ecc71;
  color:white;
  font-size:20px;
  padding:10px;
  margin: 10px 10px 0px 10px;">Succès de la modification du mot de passe</div>
                 
                 
                 
                 <div id="dlg-body_mdpmed-form-succes" style="background-color:white;
  
  color:black;
  
  font-size:14px;
  
  padding:10px;
  
  margin:0px 10px 0px 10px;">
  
                              <div style="background-color:#2ecc71; text-align:center; border-radius:10px;">
                              
                                    <br />
                                    
                                    <strong style=" font-size: 2em; color:white; ">  <i class="fa fa-check" aria-hidden="true"></i></strong> <br />
					<br />
                              
                              
                              </div>
                 
                 </div>
                 
                 <div id="dlg-footer_mdpmed-form-succes" style="background-color:#f2f2f2;
  
  text-align:right;
  padding:10px;
  margin:0px 10px 10px 10px;">
  
                           <button id="fermer_mdp_succes" style="color: white;
  background-color: #2ecc71;
  border-color: #ecf0f1;" class="btn ">Fermer</button>
             
                 
                 </div>
            
            
            
            
            </div>
            
            <c:if test="${! empty  succes_mdp }">
            
            <script src="<c:url value="/inc/jquery-1.12.4.min.js"/>"></script>
            
            
            <script type="text/javascript">
            
            	
				
				   document.getElementById("white-background-form").style.display ="block";
				   
				   document.getElementById("dlgbox_mdpmed-form-succes").style.display ="block";
				   
				   $('#fermer_mdp_succes').click(function(){
						
					   document.getElementById("white-background-form").style.display ="none";
					   
					   document.getElementById("dlgbox_mdpmed-form-succes").style.display ="none";
					   
					   
				   });
				   
				   var mdp_succes = new SpeechSynthesisUtterance("Succès de la modification du mot de passe");

				  
				   mdp_succes.lang="fr-FR";
				   
				 
				   window.speechSynthesis.speak(mdp_succes);
				   
				  
            
            </script>
            
            
            </c:if>
            
            <div id="dlgbox_mdpmed-form-echec-difference" style="display:none;
  
   position:fixed;
    width:600px;
    z-index:999999;
    border-radius:10px;
    
    
    right:30%;
    
    top:20%;
    
    left:30%;
    
    border: 10px #E74C3C outset;">
    
    <div id="dlg-header_mdpmed-form-echec-difference" style="background-color:#E74C3C;
  color:white;
  font-size:20px;
  padding:10px;
  margin: 10px 10px 0px 10px;">Échec de la modification du mot de passe</div>
                 
                 <div id="dlg-body_mdpmed-form-echec-difference" style="background-color:white;
  
  color:white;
  
  font-size: 1.5em;
  
  
  
  padding:10px;
  
  margin:0px 10px 0px 10px;">
  
  
        <div style="background-color:#E74C3C; text-align:center; border-radius:10px;">
        <br>
        
        Cause : Les mots de passe saisis sont différents.
                              
                                    <br />
                                    
                                    <strong style=" font-size: 2em; color:white; ">  <i class="fa fa-times" aria-hidden="true"></i></strong> <br />
					<br />
                              
                              
                              </div>
  
  
  </div>
                 
                 <div id="dlg-footer_mdpmed-form-echec-difference" style="background-color:#f2f2f2;
  
  text-align:right;
  padding:10px;
  margin:0px 10px 10px 10px;">
  
              <button id="fermer_mdp_echec-difference" style="color: white;
  background-color: #E74C3C;
  border-color: #E74C3C;" class="btn ">Fermer</button> 
  
  </div>
            
            
            
            
            
            
            
            
            </div>
            
            <c:if test="${! empty  differencemdp }">
            
            <script src="<c:url value="/inc/jquery-1.12.4.min.js"/>"></script>
            
            <script type="text/javascript">
            
            	
				
				   document.getElementById("white-background-form").style.display ="block";
				   
				   document.getElementById("dlgbox_mdpmed-form-echec-difference").style.display ="block";
				   
				   $('#fermer_mdp_echec-difference').click(function(){
						
					   document.getElementById("white-background-form").style.display ="none";
					   
					   document.getElementById("dlgbox_mdpmed-form-echec-difference").style.display ="none";
					   
					   
				   });
				   
				   var mdp_echec_difference_un = new SpeechSynthesisUtterance("Échec de la modification du mot de passe");

				   var mdp_echec_difference_deux = new SpeechSynthesisUtterance("Cause : Les mots de passe saisis sont différents.");

				   mdp_echec_difference_deux.lang="fr-FR";
				   
				   mdp_echec_difference_un.lang="fr-FR";
				   
				   
				   window.speechSynthesis.speak(mdp_echec_difference_un);
				   
				   window.speechSynthesis.speak(mdp_echec_difference_deux);
				    
				    
            
            </script>
            
            
            </c:if>
            
            
            <div id="dlgbox_mdpmed-form-echec-quatre" style="display:none;
  
   position:fixed;
    width:600px;
    z-index:999999;
    border-radius:10px;
    
    
    right:30%;
    
    top:20%;
    
    left:30%;
    
    border: 10px #E74C3C outset;">
    
    <div id="dlg-header_mdpmed-form-echec-quatre" style="background-color:#E74C3C;
  color:white;
  font-size:20px;
  padding:10px;
  margin: 10px 10px 0px 10px;">Échec de la modification du mot de passe</div>
                 
                 <div id="dlg-body_mdpmed-form-echec-quatre" style="background-color:white;
  
  color:white;
  
  font-size: 1.5em;
  
  
  
  padding:10px;
  
  margin:0px 10px 0px 10px;">
  
  
        <div style="background-color:#E74C3C; text-align:center; border-radius:10px;">
        <br>
        
        Cause : Les mots de passe contiennent moins de 4 caractères.
                              
                                    <br />
                                    
                                    <strong style=" font-size: 2em; color:white; ">  <i class="fa fa-times" aria-hidden="true"></i></strong> <br />
					<br />
                              
                              
                              </div>
  
  
  </div>
                 
                 <div id="dlg-footer_mdpmed-form-echec-quatre" style="background-color:#f2f2f2;
  
  text-align:right;
  padding:10px;
  margin:0px 10px 10px 10px;">
  
              <button id="fermer_mdp_echec-quatre" style="color: white;
  background-color: #E74C3C;
  border-color: #E74C3C;" class="btn ">Fermer</button> 
  
  </div>
            
            
            
            
            
            
            
            </div>
            
            <c:if test="${! empty  quatremdp }">
            
            <script src="<c:url value="/inc/jquery-1.12.4.min.js"/>"></script>
            
            <script type="text/javascript">
            
            	
				
				   document.getElementById("white-background-form").style.display ="block";
				   
				   document.getElementById("dlgbox_mdpmed-form-echec-quatre").style.display ="block";
				   
				   $('#fermer_mdp_echec-quatre').click(function(){
						
					   document.getElementById("white-background-form").style.display ="none";
					   
					   document.getElementById("dlgbox_mdpmed-form-echec-quatre").style.display ="none";
					   
					   
				   });
				   
				   var mdp_echec_quatre_un = new SpeechSynthesisUtterance("Échec de la modification du mot de passe");

				   var mdp_echec_quatre_deux = new SpeechSynthesisUtterance("Cause : Les mots de passe contiennent moins de 4 caractères.");

				   mdp_echec_quatre_deux.lang="fr-FR";
				   
				   mdp_echec_quatre_un.lang="fr-FR";
				   
				   
				   window.speechSynthesis.speak(mdp_echec_quatre_un);
				   
				   window.speechSynthesis.speak(mdp_echec_quatre_deux);
				    
            
            </script>
            
            
            </c:if>
                    <div class="enad_ENAD">
                    
                    Profil du médecin 
                    
                    <c:if test="${ ! empty sessionScope.sessionMedecin }">
${sessionScope.sessionMedecin.prenoms_medecin }  ${sessionScope.sessionMedecin.noms_medecin }


</c:if>
                    
                    </div>
            </div>
      
      
      
      </div>
      
      
      <c:import url="/inc/presentationMedecin.jsp"/>
      
      <br>
      
      <!-- MENU + CONTACTS + NOTIFS -->
      
      <div class="row">
      
           <!-- CONTACTS -->
           
           <c:import url="/inc/contactsmed.jsp" />
           
           <!-- MENU -->
           
           <div class="col-xs-8">
           
                   <div class="menumed">
                   
                   
                   <div class="row">

						<div class="col-xs-5 ">

							<div class="boutontestsetresultats">


								<button   class="btn-block buttonsmenu" id="nomsmed"
									><i
									class="fa fa-pencil"></i><br />Noms</button>
									
									<script type="text/javascript">
									
									$('#nomsmed').click(function(){
										
										
										
										   document.getElementById("white-background-form").style.display ="block";
										   
										   document.getElementById("dlgbox_nomsmed-form").style.display ="block";
										
										     
										
										
									});
									
									$('#fermerdlgboxnomsmed,#fermerhautnomsmed').click(function(){
										
										document.getElementById("white-background-form").style.display ="none";
										   
										   document.getElementById("dlgbox_nomsmed-form").style.display ="none";
										
										
										
									});
									
									
									
									</script>
									
									





							</div>



						</div>
						
						<c:if test="${! empty formnoms}">
						
						<script src="<c:url value="/inc/jquery-1.12.4.min.js"/>"></script>

	<script src="<c:url value="/inc/jquery-ui/jquery-ui.js"/>"></script>
	<script src="<c:url value="/inc/bootstrap/js/bootstrap.js"/>"></script>
						
						<script type="text/javascript">
						
						
						
						$(window).load(function(){
					        $('#nomsmed').modal('show');
					    });
						
						
						</script>
						
						</c:if>



						<div class=" col-xs-5 col-xs-push-2">

							<div class="boutoncarnetsdesante">

								<button class="btn-block buttonsmenu" id="prenomsmed"  
									><i
									class="fa fa-pencil"></i><br />Prénoms</button>
									
									
									<script type="text/javascript">
									
									$('#prenomsmed').click(function(){
										
										
										
										   document.getElementById("white-background-form").style.display ="block";
										   
										   document.getElementById("dlgbox_prenomsmed-form").style.display ="block";
										
										     
										
										
									});
									
									$('#fermerdlgboxprenomsmed,#fermerhautprenomsmed').click(function(){
										
										document.getElementById("white-background-form").style.display ="none";
										   
										   document.getElementById("dlgbox_prenomsmed-form").style.display ="none";
										
										
										
									});
									
									
									
									</script>
                        


							</div>



						</div>


					</div>
					
					<c:if test="${! empty formprenoms}">
						
						<script src="<c:url value="/inc/jquery-1.12.4.min.js"/>"></script>

	<script src="<c:url value="/inc/jquery-ui/jquery-ui.js"/>"></script>
	<script src="<c:url value="/inc/bootstrap/js/bootstrap.js"/>"></script>
						
						<script type="text/javascript">
						
						
						
						$(window).load(function(){
					        $('#prenomsmed').modal('show');
					    });
						
						
						</script>
						
						</c:if>

					<br /> <br />
					
					

					<div class="row">

						<div class="col-xs-5 hide">

							<div class="boutontestsetresultats">

								<button class="btn-block buttonsmenu" id="identifiantmed"  
									><i
									class="fa fa-pencil"></i><br />Identifiant</button>
									
									
									
									<script type="text/javascript">
									
									$('#identifiantmed').click(function(){
										
										
										
										   document.getElementById("white-background-form").style.display ="block";
										   
										   document.getElementById("dlgbox_identifiantmed-form").style.display ="block";
										
										     
										
										
									});
									
									$('#fermerdlgboxidentifiantmed,#fermerhautidentifiantmed').click(function(){
										
										document.getElementById("white-background-form").style.display ="none";
										   
										   document.getElementById("dlgbox_identifiantmed-form").style.display ="none";
										
										
										
									});
									
									
									
									</script>
									
									                 


							</div>


						</div>
						
						<c:if test="${! empty message}">
						
						<script src="<c:url value="/inc/jquery-1.12.4.min.js"/>"></script>

	<script src="<c:url value="/inc/jquery-ui/jquery-ui.js"/>"></script>
	<script src="<c:url value="/inc/bootstrap/js/bootstrap.js"/>"></script>
						
						<script type="text/javascript">
						
						
						
						$(window).load(function(){
					        $('#identifiantmed').modal('show');
					    });
						
						
						</script>
						
						</c:if>

						<div class="col-xs-5 col-xs-push-4">

							<div class="boutoncarnetsdesante">

								<button class="btn-block buttonsmenu" id="mdpmed"  
									><i
									class="fa fa-pencil"></i><br />Mot de passe</button>
									
									<script type="text/javascript">
									
									$('#mdpmed').click(function(){
										
										
										
										   document.getElementById("white-background-form").style.display ="block";
										   
										   document.getElementById("dlgbox_mdpmed-form").style.display ="block";
										
										     
										
										
									});
									
									$('#fermerdlgboxmdpmed,#fermerhautmdpmed').click(function(){
										
										document.getElementById("white-background-form").style.display ="none";
										   
										   document.getElementById("dlgbox_mdpmed-form").style.display ="none";
										
										
										
									});
									
									
									
									</script>
									
									
									              

							</div>



						</div>



					</div>
					
					
					
					<c:if test="${! empty formmdp}">
						
						<script src="<c:url value="/inc/jquery-1.12.4.min.js"/>"></script>

	<script src="<c:url value="/inc/jquery-ui/jquery-ui.js"/>"></script>
	<script src="<c:url value="/inc/bootstrap/js/bootstrap.js"/>"></script>
						
						<script type="text/javascript">
						
						
						
						$(window).load(function(){
					        $('#mdpmed').modal('show');
					    });
						
						
						</script>
						
						</c:if>

					<br /> <br />

					<div class="row">

						<div class="col-xs-5 ">

							<div class="boutoncommentaires">

								<a class="btn-block buttonsmenu"
									href="<c:url value="/collegues_Medecin"/>"><i
									class="fa fa-user-md"></i><br />Mes collègues</a>


							</div>


						</div>



						<div class="col-xs-5 col-xs-push-2">

							<div class="boutongroupes">

								<a class="btn-block buttonsmenu"
									href="<c:url value="/encharge_Medecin"/>"><i
									class="fa fa-group"></i><br />Mes patient(e)s</a>


							</div>



						</div>



					</div>
					
					

                   <br>
                   <br>
                   
                   
                   </div>
           
           
           </div>
           
           <!-- NOTIFS  -->

			<c:import url="/inc/notifsmed.jsp" />
           
      
      
      </div>
      
      <br /> <br /> <br /> <br /> <br /> <br /> <br /> <br /> <br />
              
              

<c:import url="/inc/footer.jsp"/>
                                    
                                    
                                    
                    </c:if>
                                    
                    <c:if test="${  action eq 'consulter'  }">
                    
                    
                          <c:import url="/inc/enadnotice.jsp"/>
              
              <div class="row">
      
            <div class="col-xs-12">
            
                    <div class="enad_ENAD">
                    
                    Profil du médecin 
                    
                    <c:if test="${ ! empty medecinconsulter }">
                    
    ${medecinconsulter.prenoms_medecin } ${medecinconsulter.noms_medecin }  


</c:if>


<c:if test="${ ! empty medecinconsulter.url_photo_medecin }">

<br>

<img class=""  style="margin:auto;" width="200" height="200" id="img" alt="ENAD" src="http://${  sessionScope.adresseserveur }:8080/enad${  medecinconsulter.url_photo_medecin }" >
                        


</c:if>

            
      
      <!-- PHOTO  -->


                    
                    </div>
            </div>
      
      
      
      </div>
      
      
      <c:import url="/inc/presentationMedecin.jsp"/>
      
      <br>
      
      <!-- MENU + CONTACTS + NOTIFS -->
      
      <div class="row">
      
           <!-- CONTACTS -->
           
           <c:import url="/inc/contactsmed.jsp" />
           
           <!-- MENU -->
           
           <div class="col-xs-8">
           
                   <div class="menumed">
                   
                     <c:if test="${ ! empty liaison }">
                     
                     
                               <c:if test="${  liaison eq 'oui' }">
                               
                                   <div class="row relationavecmedecinoui">
                                   
                                   <script type="text/javascript">
         
         var c = new SpeechSynthesisUtterance("Vous êtes en relation avec ce médecin.");
         
         c.lang='fr-FR';
         
         window.speechSynthesis.speak(c);

         
         </script>
                                   
                                       Vous êtes en relation avec ce médecin.
                                   
                                   
                                   </div>
                               
                               
                               
                               
                               </c:if>
                               
                               <c:if test="${  liaison eq 'non' }">
                               
                                      <div class="row relationavecmedecinnon">
                                      
                                      <script type="text/javascript">
         
         var d = new SpeechSynthesisUtterance("Vous n'êtes pas en relation avec ce médecin.");
         
         d.lang='fr-FR';
         
         window.speechSynthesis.speak(d);

         
         </script>
                                      
                                         Vous n'êtes pas en relation avec ce médecin.
                                         
                                         
                               
                                      </div>
                                      
                                      <br>
                                      <br>
                                      <br>
                               
                                  
                               
                                       <c:if test="${ ! empty demandeliaison }">
                                       
                                              <c:if test="${  demandeliaison eq 'oui' }">
                                              
                                              
                                                    <c:if test="${ ! empty demandeliaisonsens }">
                                                    
                                                             <c:if test="${  demandeliaisonsens eq 'gauchedroite' }">
                                                             
                                                             
                                                             
                                                             <a class="btn btn-primary btn-lg btn-block "  href="<c:url value="/confirmation_med_med?ide_em=${medecinconsulter.identifiant_medecin}&ide_dest=${sessionScope.sessionMedecin.identifiant_medecin }"/>"><span class="glyphicon glyphicon-info-sign"></span> <br/>Je confirme la demande de mise en relation de ce médecin.</a>
                                                             
                                                             </c:if>
                                                             
                                                             <c:if test="${  demandeliaisonsens eq 'droitegauche' }">
                                                             
                                                                  <div class="row demandeattentevalidation">
                                                                  
                                                                  <script type="text/javascript">
         
         var e = new SpeechSynthesisUtterance("Mise en relation  en attente de confirmation de la part de ce médecin");
         
         e.lang='fr-FR';
         
         window.speechSynthesis.speak(e);

         
         </script>
                                                                  
                                                                  Mise en relation  en attente de confirmation de la part de ce médecin
                                                    
                                                                  </div>
                                                             
                                                             
                                                             
                                                             </c:if>
                                                    
                                                    
                                                    </c:if>
                               
                               
                                             </c:if>
                                             
                                             <c:if test="${  demandeliaison eq 'non' }">
                                             
                                                    
                                             
                                             <a class="btn btn-primary btn-lg btn-block "  href="<c:url value="/etreenrelation_med_med?ide_em=${sessionScope.sessionMedecin.identifiant_medecin }&ide_dest=${medecinconsulter.identifiant_medecin}"/>"><span class="glyphicon glyphicon-info-sign"></span> <br/>Je souhaite être  en relation avec  ce médecin.</a>
                               
                               
                                             </c:if>
                                       
                                       
                                       
                                       </c:if>
                               
                               
                               </c:if>
                     
                     
                     
                     
                     
                     </c:if>
                   
                   
                   
                   </div>
           
           
           </div>
           
           <!-- NOTIFS  -->

			<c:import url="/inc/notifsmed.jsp" />
           
      
      
      </div>
      
      <br /> <br /> <br /> <br /> <br /> <br /> <br /> <br /> <br />
              
              

<c:import url="/inc/footer.jsp"/>
                                    
                                    
                                    
                     </c:if>
            
            
            
   </c:if>
       
       </c:if>
  
  
  
  </c:if>

            

              
</div>

<script src="<c:url value="/inc/jquery-1.12.4.min.js"/>"></script>

	<script src="<c:url value="/inc/jquery-ui/jquery-ui.js"/>"></script>
	<script src="<c:url value="/inc/bootstrap/js/bootstrap.js"/>"></script>


</body>
</html>