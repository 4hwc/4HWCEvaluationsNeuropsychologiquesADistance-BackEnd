 ...
 *
 */
public final class MdpOubliForm {
	...
	public MdpOubliForm(MedecinDao medecinDao) {
	...
	public Medecin mdpMedecin(HttpServletRequest request) {
		...
		try {
			...
			if (erreurs.isEmpty()) {
				...
				medecinDao.modifierMotDePasse(medecin, identifiant_correct);
	...
	private void traiterMdpMedecin(String mdpMedecin, Medecin medecin) {
		...
		medecin.setMdp_medecin(motdepasse);
		...
		if (mdpMedecin != null && mdpMedecin.trim().length() != 0) {
			...
			if (mdpMedecin.trim().length() < 4) {
				...
				if (capteur_identifiant_correct == 1) {
					...
					motdepasse = passwordEncryptor.encryptPassword(mdpMedecin);
